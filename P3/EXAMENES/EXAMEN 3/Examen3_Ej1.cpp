...
void Filosofo( int id )
{
    ...
    while ( true )
    {
        ...
        MPI_Ssend( &id, 1, MPI_INT,segundo, segundo, MPI_COMM_WORLD); //AQUI FALTABA EL 2 "SEGUNDO"
        cout << "Filosofo " << id << " coge segundo tenedor: " << segundo << endl << flush;
        ...
        // solicita levantarse
        cout << "Filosofo " << id << " solicita sentarse." << endl << flush;
        MPI_Ssend ( &id, 1, MPI_INT, RANK_CAMARERO, TAG_LEVANTARSE, MPI_COMM_WORLD ); //ESTABA PUESTO EL TAG DE SENTARSE
...
void Tenedor( int id )
{
    ...
    while ( true )
    {
        ...
        MPI_Recv(&valor, 1, MPI_INT, status.MPI_SOURCE, id, MPI_COMM_WORLD, &status ); // ESTABAN VALOR Y 1 AL REVÉS
