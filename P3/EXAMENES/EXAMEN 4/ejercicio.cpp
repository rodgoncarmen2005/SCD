...
const int tag_sentar_4 = 2, tag_levantar_4 = 3;
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    ...
    if(id != 8){
      ...
      cout <<"+++++++++++++++++++++++++  Filosofo " <<id << " solicita SENTARSE (especial)." <<endl;
      MPI_Ssend(&peticion, 1, MPI_INT,camarero,tag_sentar_4,MPI_COMM_WORLD);
    ...
    if (id != 8){
      ...
      cout <<"+++++++++++++++++++++++++++  Filosofo " <<id << " solicita LEVANTARSE (especial)." <<endl;
      MPI_Ssend(&peticion, 1, MPI_INT,camarero,tag_levantar_4,MPI_COMM_WORLD);
    }
...
{
  int sentados=0, id_filosofo, tag, valor, flag = 0; 
  MPI_Status estado ;       

  bool peticion_encontrada; // Indica si se ha encontrado alguna petición pendiente
  bool sentar = false; // Indica si la petición encontrada es para sentarse o levantarse

  while ( true )
  {
    if (sentados<4){
      // Sondeamos peticiones de sentarse o levantar (excepto filósofos 4)
      peticion_encontrada=false;

      while (!peticion_encontrada) {
        sentar = !sentar; // Alternar entre buscar sentarse y levantarse
        if (sentar) {
          // Buscar peticiones para sentarse
          MPI_Iprobe(MPI_ANY_SOURCE, tag_sentar, MPI_COMM_WORLD, &flag, &estado);
        } else {
          // Buscar peticiones para levantarse
          MPI_Iprobe(MPI_ANY_SOURCE, tag_levantar, MPI_COMM_WORLD, &flag, &estado);
        }

        if (flag > 0){
          peticion_encontrada = true;
        }
      }

      // Una vez encontrada la petición, determinamos el tag y el filósofo del que proviene
      if (sentar){
        tag = tag_sentar;
      }else{
        tag = tag_levantar; 
      ...
      if(tag != estado.MPI_TAG){
        cerr << "Error: Se esperaba tag " << tag << " pero se recibió tag " << estado.MPI_TAG << endl;
        exit(1);
      }
      ...
      // Solo se aceptan peticiones de levantarse o la de sentarse del filósofo 4
      peticion_encontrada=false;

      while (!peticion_encontrada) {
        sentar = !sentar; // Alternar entre buscar sentarse y levantarse del filósofo 4

        if (sentar)
          MPI_Iprobe(MPI_ANY_SOURCE, tag_sentar_4, MPI_COMM_WORLD, &flag, &estado);
        else
          MPI_Iprobe(MPI_ANY_SOURCE, tag_levantar, MPI_COMM_WORLD, &flag, &estado);
      ...
      // Si hay 5 sentados, solo se aceptan peticiones de levantarse del filósofo 4
      MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, tag_levantar_4, MPI_COMM_WORLD, &estado);
    }
