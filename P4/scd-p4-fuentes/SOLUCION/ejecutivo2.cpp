...
int main( int argc, char *argv[] )
{
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         // obtemeos el instante real de comienzo de la siguiente iteración
         time_point<steady_clock> instante_real = steady_clock::now();

         // calcular el retraso 
         float retraso = milliseconds_f(instante_real - ini_sec).count();
...
// El mínimo tiempo de espera que queda al final de las iteraciones del ciclo secundario es 10 ms.
/*¿ sería planificable si la tarea D tuviese un tiempo cómputo de
250 ms ?*/
// Sí, porque en el ciclo secundario 2, la suma de los tiempos de cómputo sería 100 + 150 + 250 = 500 ms, que es igual a Ts.
